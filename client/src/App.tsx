...
function CoinIcon({ size = 40 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="50" y="72" fontSize="52" textAnchor="middle" fontFamily="Segoe UI Emoji,Apple Color Emoji,Noto Color Emoji,sans-serif">👜</text>
    </svg>
  );
}

// Wordmark NaCarteira para header PC
function BrandMark() {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
      <CoinIcon size={28}/>
      <span style={{ fontSize:15, fontWeight:700, color:"var(--text)", letterSpacing:-0.3 }}>NaCarteira</span>
    </div>
  );
}
...
function DonationPopup({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300, padding:"0" }} onClick={onClose}>
      <div style={{ background:"#111520", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, overflow:"hidden", border:"0.5px solid rgba(130,10,209,0.5)", borderBottom:"none", paddingBottom:"env(safe-area-inset-bottom,0)" }} onClick={e=>e.stopPropagation()}>
        {/* Handle */}
        ...
        <div style={{ padding:"20px 24px 24px", display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ background:"rgba(255,255,255,0.04)", border:"0.5px solid rgba(255,255,255,0.1)", borderRadius:14, padding:"14px 18px" }}>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.5)", marginBottom:8, fontWeight:600 }}>CHAVE PIX · NUBANK</div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                <div style={{ fontFamily:"monospace", fontSize:15, color:"white", fontWeight:600, letterSpacing:0.5 }}>11976016950</div>
                <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginTop:3 }}>João P S Saraiva</div>
              </div>
              <button onClick={copyPix} style={{ background:"#820AD1", border:"none", color:"white", fontSize:12, fontWeight:600, padding:"8px 16px", borderRadius:8, cursor:"pointer" }}>
                {copied ? "✓ Copiado" : "Copiar"}
              </button>
            </div>
          </div>
          {/* Ko-fi */}
          <a href="https://ko-fi.com/nacarteira" target="_blank" rel="noopener noreferrer"
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, background:"rgba(255,255,255,0.06)", border:"0.5px solid rgba(255,255,255,0.12)", color:"rgba(255,255,255,0.8)", fontSize:13, fontWeight:600, padding:"13px", borderRadius:12, textDecoration:"none" }}>
            ☕ Pagar via Ko-fi (cartão aceito)
          </a>
...
function useDonationPopup(createdAt?: string) {
  ...
  useEffect(() => {
    ...
    if (parseInt(localStorage.getItem(monthKey) || "0") >= 6) return;
    const t = setTimeout(() => {
      setShow(true);
...
function CollapsibleSection({ title, sub, defaultOpen=false, children }: { title:string; sub?:string; defaultOpen?:boolean; children:React.ReactNode }) {
  ...
  return (
    <div style={{ background:"var(--bg2)", borderRadius:14, overflow:"hidden", marginBottom:6, border:"0.5px solid rgba(255,255,255,0.05)" }}>
      ...
      {open && <div style={{ borderTop:"0.5px solid rgba(255,255,255,0.06)" }}>{children}</div>}
    </div>
  );
}
      ...
      <div style={{ display:"flex", height:"100vh", overflow:"hidden", background:"var(--bg)" }}>
        <aside style={{ width:235, background:"var(--bg2)", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
          <div style={{ padding:"22px 18px 14px", borderBottom:"1px solid var(--border)" }}>
            <BrandMark/>
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.35)", marginTop:6 }}>Olá, {user.nickname || user.name?.split(" ")[0]} {user.emoji || "👜"}</div>
          </div>

          <nav style={{ padding:"12px 10px", flex:1 }}>
            {NAV.map(n=>(
              <button key={n.id} onClick={()=>setTab(n.id)} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"9px 13px", borderRadius:10, marginBottom:2, background:tab===n.id?"rgba(130,10,209,0.15)":"transparent", border:`0.5px solid ${tab===n.id?"rgba(130,10,209,0.35)":"transparent"}`, color:tab===n.id?"#c084fc":"rgba(255,255,255,0.45)", fontSize:13, fontWeight:tab===n.id?600:400, cursor:"pointer", textAlign:"left", transition:"all 0.15s" }}>
                <span style={{ fontSize:15 }}>{n.icon}</span> {n.label}
              </button>
            ))}
          </nav>
          <div style={{ padding:"10px 10px 20px", borderTop:"1px solid var(--border)", display:"flex", flexDirection:"column", gap:6 }}>
            <button onClick={()=>setShowDonation(true)} style={{ width:"100%", background:"linear-gradient(135deg,rgba(130,10,209,0.18),rgba(130,10,209,0.08))", border:"1px solid rgba(130,10,209,0.35)", color:"#c084fc", padding:"10px 12px", borderRadius:10, fontSize:12, textAlign:"left", cursor:"pointer", fontWeight:600 }}>☕ Apoie o NaCarteira</button>
            ...
            <button onClick={logout} style={{ width:"100%", background:"rgba(255,77,106,0.06)", border:"1px solid rgba(255,77,106,0.15)", color:"var(--red)", padding:"9px 12px", borderRadius:10, fontSize:12, textAlign:"left", cursor:"pointer" }}>🚪 Sair</button>
          </div>
        </aside>
      ...
      <main style={{ padding:"0 12px" }}>
        ...
        {tab==="dashboard" && (
          <>
            ...
            <button onClick={()=>setShowDonation(true)} style={{ width:"100%", background:"linear-gradient(135deg,rgba(130,10,209,0.12),rgba(130,10,209,0.06))", border:"0.5px solid rgba(130,10,209,0.35)", borderRadius:12, padding:"13px 16px", display:"flex", alignItems:"center", gap:10, cursor:"pointer", marginBottom:10 }}>
              <div style={{ width:32, height:32, borderRadius:"50%", background:"rgba(130,10,209,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>☕</div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:600, color:"#c084fc" }}>Apoie o NaCarteira</div>
                <div style={{ fontSize:11, color:"rgba(192,132,252,0.7)", marginTop:1 }}>Pague um Ko-fi ao criador</div>
              </div>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#c084fc" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
          </>
        )}
        {tab==="expenses"&&<ExpensesContent expenses={expenses} byCategory={byCategory} onAdd={()=>setShowAddExp(true)} onPay={async(exp:Expense)=>{ await apiFetch(`${API}/expenses/${exp.id}/paid`,{method:"PATCH",body:JSON.stringify({paid:!exp.paid})}); if(!exp.paid)gainXpRaw(XP_PAY_BILL); load(); }} onDelete={async(id:number)=>{ await apiFetch(`${API}/expenses/${id}`,{method:"DELETE"}); load(); }} onCreditTab={()=>setTab("credit")}/>}
...
// ── ABA DESPESAS ──────────────────────────────────────────────────────────────
function ExpensesContent({ expenses,byCategory,onAdd,onPay,onDelete,onCreditTab }: any) {
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        <div style={{ fontSize:15, fontWeight:700, color:"var(--text)" }}>Despesas</div>
        <button className="btn-primary" onClick={onAdd} style={{ padding:"8px 14px", fontSize:13 }}>+ Adicionar</button>
      </div>
      {/* Acesso rápido ao Cartão */}
      {onCreditTab && (
        <button onClick={onCreditTab} style={{ width:"100%", background:"var(--bg2)", border:"0.5px solid rgba(167,139,250,0.3)", borderRadius:12, padding:"11px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer", marginBottom:12 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:"50%", background:"rgba(167,139,250,0.12)", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2" strokeLinecap="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
            </div>
            <span style={{ fontSize:13, color:"var(--text2)" }}>Cartão de crédito</span>
          </div>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a78bfa" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
      )}
...
// ── RELATÓRIOS: tipos de card IA ───────────────────────────────────────────────
const AI_CARD_STYLES: Record<string,{border:string;labelBg:string;labelColor:string;barColor:string;label:string}> = {
  alert_critico: { border:"rgba(255,77,106,0.4)",  labelBg:"rgba(255,77,106,0.15)",  labelColor:"#ff4d6a", barColor:"#ff4d6a", label:"⚠ ALERTA CRÍTICO" },
  alerta:        { border:"rgba(255,183,3,0.4)",   labelBg:"rgba(255,183,3,0.12)",   labelColor:"#ffd700", barColor:"#ffd700", label:"⚡ ALERTA" },
  ponto_forte:   { border:"rgba(0,214,143,0.4)",   labelBg:"rgba(0,214,143,0.12)",   labelColor:"#00d68f", barColor:"#00d68f", label:"✦ PONTO FORTE" },
  forca_oculta:  { border:"rgba(6,182,212,0.4)",   labelBg:"rgba(6,182,212,0.12)",   labelColor:"#06b6d4", barColor:"#06b6d4", label:"◎ FORÇA OCULTA" },
  curto_prazo:   { border:"rgba(130,10,209,0.4)",  labelBg:"rgba(130,10,209,0.12)",  labelColor:"#c084fc", barColor:"#820AD1", label:"◷ PROJEÇÃO CURTO PRAZO" },
  longo_prazo:   { border:"rgba(167,139,250,0.35)",labelBg:"rgba(167,139,250,0.1)",  labelColor:"#a78bfa", barColor:"#a78bfa", label:"◑ PROJEÇÃO LONGO PRAZO" },
};

function AICard({ type, title, body, quote, quoteAuthor }: { type:string;title:string;body:string;quote?:string;quoteAuthor?:string }) {
  const s = AI_CARD_STYLES[type] || AI_CARD_STYLES.alerta;
  return (
    <div style={{ background:"var(--bg2)", border:`0.5px solid ${s.border}`, borderRadius:14, overflow:"hidden", marginBottom:10 }}>
      <div style={{ borderLeft:`3px solid ${s.barColor}`, padding:"14px 16px" }}>
        <div style={{ display:"inline-block", background:s.labelBg, color:s.labelColor, fontSize:10, fontWeight:700, letterSpacing:1.2, padding:"3px 8px", borderRadius:4, marginBottom:10, textTransform:"uppercase" as any }}>{s.label}</div>
        <div style={{ fontSize:14, fontWeight:600, color:"var(--text)", marginBottom:6, lineHeight:1.4 }}>{title}</div>
        ...
        {quote && (
          <div style={{ marginTop:10, paddingTop:10, borderTop:"0.5px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontSize:12, color:"var(--text2)", fontStyle:"italic", lineHeight:1.55 }}>"{quote}"</div>
            {quoteAuthor && <div style={{ fontSize:11, color:s.labelColor, marginTop:4, fontWeight:600 }}>— {quoteAuthor}</div>}
          </div>
        )}
      </div>
    </div>
...
function ReportsContent({ byCategory,totalExpSemSonho,totalCC,totalIncome,expenses,cc,xp,userId,salary,healthScore }: any) {
  ...
  // IA automática 1x por semana
  const weekKey = `nc_report_week_${Math.floor(Date.now()/(7*24*3600*1000))}_${userId}`;
  useEffect(()=>{
    try {
      const cached = localStorage.getItem(weekKey);
      if (cached) { setAiCards(JSON.parse(cached)); return; }
    } catch {}
    // Gera automaticamente se não houver cache
    if (salary <= 0 && totalExpSemSonho+totalCC <= 0) return;
    setLoadingAI(true);
    const totalGasto = totalExpSemSonho + totalCC;
    const saldo = salary + totalIncome - totalGasto;
    const pctCC = (salary+totalIncome)>0 ? Math.round(totalCC/(salary+totalIncome)*100) : 0;
    const prompt = `Você é um consultor financeiro do app NaCarteira (metodologia dos 6 potes). Com base nos dados abaixo, gere uma análise financeira em JSON com exatamente 6 cards.

DADOS DO MÊS:
- Salário: R$ ${salary.toFixed(2)}
- Renda extra: R$ ${totalIncome.toFixed(2)}
- Total despesas: R$ ${totalGasto.toFixed(2)}
- Cartão de crédito: R$ ${totalCC.toFixed(2)} (${pctCC}% da receita)
- Saldo livre: R$ ${saldo.toFixed(2)}
- Saúde financeira: ${healthScore}/100

Responda SOMENTE com JSON válido, sem markdown. Formato exato:
[
  {"type":"alert_critico","title":"título curto","body":"2-3 frases diretas com números reais","quote":"frase motivacional curta","quoteAuthor":"nome"},
  {"type":"alerta","title":"...","body":"...","quote":"...","quoteAuthor":"..."},
  {"type":"ponto_forte","title":"...","body":"...","quote":"...","quoteAuthor":"..."},
  {"type":"forca_oculta","title":"...","body":"...","quote":"...","quoteAuthor":"..."},
  {"type":"curto_prazo","title":"...","body":"...","quote":"...","quoteAuthor":"..."},
  {"type":"longo_prazo","title":"...","body":"...","quote":"...","quoteAuthor":"..."}
]`;

    apiFetch(`${API}/ai/insights`, { method:"POST", body: JSON.stringify({ prompt }) })
      .then(r=>r.json())
      .then(d=>{
  ...
  return (
    <div>
      <div style={{ fontSize:16, fontWeight:700, color:"var(--text)", marginBottom:16 }}>Relatórios</div>

      {/* IA CARDS — carregamento automático */}
      {loadingAI && (
        <div style={{ background:"var(--bg2)", borderRadius:14, padding:"20px 16px", textAlign:"center", marginBottom:10 }}>
          <div style={{ fontSize:12, color:"var(--text2)" }}>Gerando análise da semana...</div>
        </div>
      )}
      {aiCards && aiCards.map((card:any, i:number) => (
        <AICard key={i} type={card.type} title={card.title} body={card.body} quote={card.quote} quoteAuthor={card.quoteAuthor}/>
      ))}
      {aiCards && (
        <div style={{ fontSize:10, color:"rgba(255,255,255,0.2)", textAlign:"center", marginBottom:14 }}>
          Análise gerada por IA · atualiza semanalmente
        </div>
      )}

      {/* HISTÓRICO DE MESES — gráfico corrigido com 3 barras */}
      <div style={{ background:"var(--bg2)", borderRadius:14, padding:"16px", marginBottom:10 }}>
        <div style={{ fontSize:13, fontWeight:600, color:"var(--text)", marginBottom:16 }}>📅 Histórico de Meses</div>
        {loadingH ? (
          <div style={{ textAlign:"center", color:"var(--text2)", padding:16, fontSize:13 }}>Carregando...</div>
        ) : history.length===0 ? (
          <div style={{ textAlign:"center", color:"var(--text2)", padding:16, fontSize:13, lineHeight:1.6 }}>
            Nenhum mês arquivado.<br/>
            <span style={{ fontSize:11 }}>Use "Virar Mês" em Configurações.</span>
          </div>
        ) : (
          <>
            {/* Gráfico: 3 barras por mês (salário, renda extra, despesas) */}
            {(() => {
              const data = history.slice().reverse();
              const maxVal = Math.max(...data.flatMap((h:any)=>[h.totalIncome||0, h.totalExpenses||0]), 1);
              const BAR_H = 120;
              return (
                <div style={{ overflowX:"auto", marginBottom:16 }}>
                  <div style={{ minWidth: Math.max(data.length*70, 240), padding:"0 4px" }}>
                    {/* Grade */}
                    <div style={{ position:"relative", height:BAR_H, marginBottom:0 }}>
                      {[0.25,0.5,0.75,1].map((p,i)=>(
                        <div key={i} style={{ position:"absolute", left:0, right:0, bottom:`${p*BAR_H}px`, borderTop:"0.5px dashed rgba(255,255,255,0.06)" }}>
                          <span style={{ fontSize:8, color:"rgba(255,255,255,0.2)", paddingLeft:2 }}>
                            {fmt(maxVal*p).replace("R$ ","").replace(",00","")}
                          </span>
                        </div>
                      ))}
                      {/* Barras */}
                      <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"flex-end", gap:0 }}>
                        {data.map((h:any, i:number) => {
                          const salBase = Math.max(0, h.totalIncome - (h.totalIncome * 0.3));
                          const rendaExtra = h.totalIncome - salBase;
                          const hSal = Math.max(salBase/maxVal*BAR_H, salBase>0?3:0);
                          const hExtra = Math.max(rendaExtra/maxVal*BAR_H, rendaExtra>0?3:0);
                          const hExp = Math.max(h.totalExpenses/maxVal*BAR_H, h.totalExpenses>0?3:0);
                          return (
                            <div key={i} style={{ flex:1, display:"flex", alignItems:"flex-end", justifyContent:"center", gap:3, paddingBottom:0 }}>
                              <div title={`Salário: ${fmt(salBase)}`} style={{ width:10, height:hSal, background:"#4f8ef7", borderRadius:"2px 2px 0 0", opacity:0.9 }}/>
                              <div title={`Renda extra: ${fmt(rendaExtra)}`} style={{ width:10, height:hExtra, background:"#00d68f", borderRadius:"2px 2px 0 0", opacity:0.9 }}/>
                              <div title={`Despesas: ${fmt(h.totalExpenses)}`} style={{ width:10, height:hExp, background:"#ff4d6a", borderRadius:"2px 2px 0 0", opacity:0.9 }}/>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {/* Labels meses */}
                    <div style={{ display:"flex", borderTop:"0.5px solid rgba(255,255,255,0.08)", paddingTop:6 }}>
                      {data.map((h:any,i:number)=>(
                        <div key={i} style={{ flex:1, textAlign:"center", fontSize:9, color:"rgba(255,255,255,0.35)", textTransform:"capitalize" }}>
                          {new Date(h.month+"-02").toLocaleDateString("pt-BR",{month:"short",year:"2-digit"})}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ display:"flex", gap:16, justifyContent:"center", marginTop:10 }}>
                    {[{color:"#4f8ef7",label:"Salário fixo"},{color:"#00d68f",label:"Renda extra"},{color:"#ff4d6a",label:"Despesas"}].map((l,i)=>(
                      <div key={i} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"rgba(255,255,255,0.4)" }}>
            ...
            {history.map((h:any,i:number)=>(
              <div key={i} style={{ padding:"12px 14px", background:"var(--bg3)", borderRadius:10, marginBottom:7, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <div>
                  ...
                  <div style={{ display:"flex", gap:12, fontSize:11, color:"var(--text2)", marginTop:3 }}>
                    <span style={{ color:"#4f8ef7" }}>⬛ {fmt(h.totalIncome)}</span>
                    <span style={{ color:"#ff4d6a" }}>⬛ {fmt(h.totalExpenses)}</span>
                  </div>
                </div>
                <div style={{ fontSize:14, fontWeight:700, color:h.balance>=0?"var(--green)":"var(--red)", fontVariantNumeric:"tabular-nums" }}>
      ...
      {/* DISTRIBUIÇÃO POR POTE */}
      <div style={{ background:"var(--bg2)", borderRadius:14, padding:"16px", marginBottom:10 }}>
        <div style={{ fontSize:13, fontWeight:600, color:"var(--text)", marginBottom:12 }}>Gastos por pote — {monthLabel}</div>
        {byCategory.filter((c:any)=>c.total>0).map((cat:any)=>(
          <div key={cat.id} style={{ marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:5, color:"var(--text2)" }}>
              <span>{cat.emoji} {cat.name}</span>
              <span style={{ fontWeight:600, color:"var(--text)", fontVariantNumeric:"tabular-nums" }}>{fmt(cat.total)} · {totalGasto>0?Math.round(cat.total/totalGasto*100):0}%</span>
            </div>
            <div style={{ height:3, background:"rgba(255,255,255,0.07)", borderRadius:2, overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${totalGasto>0?Math.min(cat.total/totalGasto*100,100):0}%`, background:cat.color, borderRadius:2, transition:"width .6s" }}/>
            </div>
          </div>
        ))}
        {byCategory.every((c:any)=>c.total===0)&&<div style={{ textAlign:"center", color:"var(--text2)", padding:12, fontSize:13 }}>Nenhum gasto registrado ainda</div>}
      </div>
    </div>
  );
}
