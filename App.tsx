...
// Nova fórmula: dia 1=10xp, dia 2=15xp, dia 3=20xp... +5xp por dia
...
function StreakModal({ user, onClose, onClaim }: { user:User; onClose:()=>void; onClaim:(d:any)=>void }) {
  ...
  const nextXP = getStreakXP(nextDay);
  const accent = streakDays >= 30 ? "#ffd700" : streakDays >= 14 ? "#a78bfa" : streakDays >= 7 ? "#6c63ff" : "#f97316";

  // Milestones do mês (dias chave)
  const milestones = [
    { day:1,  emoji:"💸", xp:getStreakXP(1)  },
    { day:3,  emoji:"💸", xp:getStreakXP(3)  },
    { day:7,  emoji:"⚡",  xp:getStreakXP(7)  },
    { day:14, emoji:"💎", xp:getStreakXP(14) },
    { day:21, emoji:"🚀", xp:getStreakXP(21) },
    { day:28, emoji:"👑", xp:getStreakXP(28) },
    { day:31, emoji:"🏆", xp:getStreakXP(31) },
  ];
          ...
          {milestones.map((m,i)=>{
            ...
            return (
              <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2, minWidth:36,
                background: isCurrent ? "rgba(130,10,209,0.25)" : active ? "rgba(130,10,209,0.10)" : "var(--bg3)",
                border: `1.5px solid ${isCurrent ? "#820AD1" : active ? "rgba(130,10,209,0.3)" : "var(--border)"}`,
                borderRadius:10, padding:"6px 4px" }}>
                <span style={{ fontSize:15 }}>{m.emoji}</span>
                <span style={{ fontSize:10, fontWeight:800, color: active ? "var(--text)" : "var(--text2)" }}>{m.day}</span>
...
function Auth({ onLogin }: { onLogin:(u:User)=>void }) {
  ...
  const submitReset = async () => {
    setError("");
    if (!resetForm.email || !resetForm.newPassword) { setError("Preencha todos os campos"); return; }
    if (resetForm.newPassword !== resetForm.confirm) { setError("As senhas não coincidem"); return; }
    if (resetForm.newPassword.length < 4) { setError("Senha deve ter ao menos 4 caracteres"); return; }
    setLoading(true);
    try {
      const res = await fetch(`${API}/auth/reset-password`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:resetForm.email,newPassword:resetForm.newPassword})});
      const data = await res.json();
      if (!res.ok) { setError(data.error||"E-mail não encontrado"); return; }
        ...
        <div className="card">
          {resetSuccess ? (
            <div style={{ textAlign:"center", padding:"10px 0" }}>
              ...
              <div style={{ fontSize:16, fontWeight:800, color:"var(--green)", marginBottom:6 }}>Senha alterada!</div>
              <div style={{ fontSize:13, color:"var(--text2)", marginBottom:20 }}>Agora entre com sua nova senha.</div>
              <button className="btn-primary" onClick={()=>{ setMode("login"); setResetSuccess(false); setResetForm({email:"",newPassword:"",confirm:""}); }} style={{ width:"100%" }}>Ir para o login</button>
            </div>
          ) : (
            <>
              <div style={{ fontSize:15, fontWeight:800, marginBottom:4 }}>🔑 Redefinir senha</div>
              <div style={{ fontSize:12, color:"var(--text2)", marginBottom:18 }}>Informe seu e-mail e escolha uma nova senha.</div>
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                <input type="email" placeholder="Seu e-mail cadastrado" value={resetForm.email} onChange={e=>setResetForm(f=>({...f,email:e.target.value}))}/>
                <input type="password" placeholder="Nova senha" value={resetForm.newPassword} onChange={e=>setResetForm(f=>({...f,newPassword:e.target.value}))}/>
                <input type="password" placeholder="Confirmar nova senha" value={resetForm.confirm} onChange={e=>setResetForm(f=>({...f,confirm:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&submitReset()}/>
                {error&&<p style={{ color:"var(--red)", fontSize:13, textAlign:"center" }}>{error}</p>}
                <button className="btn-primary" onClick={submitReset} disabled={loading} style={{ width:"100%", marginTop:4 }}>{loading?"Salvando...":"Salvar nova senha"}</button>
      ...
      <div style={{ width:"100%", maxWidth:370 }}>
        ...
        <div className="card">
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            {mode==="login" && (
              <button onClick={()=>{ setMode("reset"); setError(""); }} style={{ background:"none", border:"none", color:"var(--text2)", fontSize:12, cursor:"pointer", textDecoration:"underline", padding:"2px 0" }}>
                Esqueci minha senha
              </button>
            )}
...
function DashboardContent({ expenses,cc,incomes,salary,balance,totalExpSemSonho,totalExpReais,totalInvestido,totalCC,totalIncome,totalPaid,totalPending,extraNeeded,sonhoTotal,sonhoPago,sonhoRecorrente,sonhoProgresso,byCategory,streakDays,streakClaimed,healthScore,levelInfo,onStreak,onCreditClick }: any) {
  return (
    <>
      ...
      {/* KPIs — Despesas Reais (sem Investir) */}
      ...
      {/* INVESTIMENTO — separado das despesas */}
      {(()=>{
        ...
        // Dica: combina % investido atual com o tier do investidor
        type RiskProfile = { label:string; color:string; icon:string; tip:string; tipExtra?: string };
        let riskProfile: RiskProfile;
        if (tier === "avancado") {
          if (pct >= 20) riskProfile = { label:"Agressivo", color:"#a78bfa", icon:"🚀", tip:"Você está no topo. Considere diversificar em FIIs, BDRs e fundos internacionais para reduzir risco concentrado.", tipExtra:"Verifique se sua reserva de emergência está intacta — ela nunca deve ser investida." };
          else if (pct >= 10) riskProfile = { label:"Moderado", color:"#00d68f", icon:"📈", tip:"Bom ritmo de quem já sabe o que faz. Para seu nível, explore fundos de índice (ETFs) e previdência privada PGBL/VGBL.", tipExtra:`Seu target como Avançado é ${targetPct.investir}% — você está em ${pct}%.` };
          else riskProfile = { label:"Abaixo do target", color:"#ffb703", icon:"⚠️", tip:"Para seu nível, você pode aportar mais.", tipExtra:`Meta recomendada para Avançado: ${targetPct.investir}% da receita. Você está em ${pct}%.` };
        } else if (tier === "investidor") {
          if (pct >= 15) riskProfile = { label:"Acima do Target", color:"#a78bfa", icon:"🚀", tip:"Excelente! Você está superando o target do seu nível. Considere CDBs de liquidez diária e LCI/LCA isentos de IR.", tipExtra:"Diversifique: não coloque mais de 30% em um único ativo ou emissor." };
          else if (pct >= 10) riskProfile = { label:"No Target", color:"#00d68f", icon:"✅", tip:`Perfeito para o nível Investidor. Foque em Tesouro Selic para sua reserva e comece a explorar ações de dividendos.`, tipExtra:`Sua meta é ${targetPct.investir}% — você está em ${pct}%. Continue assim.` };
          else riskProfile = { label:"Conservador", color:"#ffb703", icon:"🛡️", tip:"Boa base! Priorize Tesouro Selic até completar sua reserva de emergência (6× as despesas mensais).", tipExtra:`Meta do nível Investidor: ${targetPct.investir}% — você está em ${pct}%. Pequenos aumentos mensais fazem diferença.` };
        } else {
          // Iniciante
          if (pct >= 10) riskProfile = { label:"Acima do esperado!", color:"#00d68f", icon:"🌱", tip:"Incrível para um Iniciante! Certifique-se de ter uma reserva de emergência antes de assumir mais risco.", tipExtra:"Chegando a 100 XP você vira Investidor e recebe recomendações mais sofisticadas." };
          else if (pct >= 5) riskProfile = { label:"No caminho certo", color:"#6c63ff", icon:"💡", tip:"Ótimo começo. Tesouro Selic e CDBs de alta liquidez são perfeitos para quem está construindo disciplina.", tipExtra:`Meta do nível Iniciante: ${targetPct.investir}% — você está em ${pct}%. Suba de nível para desbloquear mais dicas.` };
          else riskProfile = { label:"Comece pequeno", color:"#ffb703", icon:"🌱", tip:"Mesmo R$50/mês investidos hoje, com juros compostos, valem muito mais em 10 anos. Consistência > volume.", tipExtra:"Dica: automatize um aporte fixo todo dia de pagamento — o que você não vê, não gasta." };
        }
        ...
        return (
          <div style={{ background:"linear-gradient(135deg,rgba(0,214,143,0.07),rgba(0,214,143,0.03))", border:"1px solid rgba(0,214,143,0.28)", borderRadius:14, padding:"13px 16px", marginBottom:14 }}>
            ...
            {/* Barra de progresso em direção ao target */}
            {receita > 0 && (
...
export default function App() {
  ...
  const NAV = [{id:"dashboard",icon:"💸",label:"Dashboard"},{id:"expenses",icon:"💸",label:"Despesas"},{id:"credit",icon:"💳",label:"Cartão"},{id:"income",icon:"💵",label:"Renda Extra"},{id:"reports",icon:"📈",label:"Relatórios"},{id:"ranking",icon:"🏆",label:"Ranking"}];

  // Carrega emoji/nick do usuário
  useEffect(() => {
    const stored = localStorage.getItem(`mg_identity_${user?.id}`);
    if (stored) { try { const p = JSON.parse(stored); setUserEmoji(p.emoji||"💸"); setUserNick(p.nick||""); } catch {} }
  }, [user?.id]);
  ...
  const sharedModals = (
    <>
      ...
      {showSettings&&<SettingsModal user={user} salary={salary} userEmoji={userEmoji} userNick={userNick}
        onSave={(s:number)=>{setSalary(s);showToast("✅ Salário atualizado!");setShowSettings(false);}}
        onClose={()=>setShowSettings(false)} onReset={()=>{setShowSettings(false);setShowReset(true);}}
      ...
      {showEmojiPicker&&<EmojiPickerModal current={{emoji:userEmoji,nick:userNick}} onClose={()=>setShowEmojiPicker(false)} onSave={(e,n)=>{ setUserEmoji(e); setUserNick(n); localStorage.setItem(`mg_identity_${user.id}`,JSON.stringify({emoji:e,nick:n})); fetch(`${API}/users/${user.id}/settings`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({emoji:e,nickname:n})}); setShowEmojiPicker(false); showToast("✅ Identidade atualizada!"); }}/>}
    </>
  );
      ...
      <div style={{ display:"flex", height:"100vh", overflow:"hidden", background:"var(--bg)" }}>
        <aside style={{ width:235, background:"var(--bg2)", borderRight:"1px solid var(--border)", display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
          ...
          <div style={{ padding:"10px 10px 20px", borderTop:"1px solid var(--border)", display:"flex", flexDirection:"column", gap:6 }}>
            ...
            <button onClick={()=>setShowEmojiPicker(true)} style={{ width:"100%", background:"var(--bg3)", border:"1px solid var(--border)", color:"var(--text2)", padding:"9px 12px", borderRadius:10, fontSize:12, textAlign:"left", cursor:"pointer" }}>{userEmoji} {userNick || "Minha identidade"}</button>
  ...
  return (
    <div style={{ minHeight:"100vh", paddingBottom:80 }}>
      ...
      <nav style={{ position:"fixed", bottom:0, left:0, right:0, background:"var(--bg2)", borderTop:"1px solid var(--border)", display:"flex", zIndex:50 }}>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>setTab(n.id)} style={{ flex:1, padding:"9px 4px 7px", background:"none", border:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:2, color:tab===n.id?"#820AD1":"var(--text2)", cursor:"pointer" }}>
...
function DonationPopup({ onClose }: { onClose: () => void }) {
  ...
  const copyPix = () => {
    navigator.clipboard.writeText("11976016950").then(() => {
      setCopied(true);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300, padding:20 }} onClick={onClose}>
      <div style={{ background:"var(--bg2)", borderRadius:20, width:"100%", maxWidth:380, overflow:"hidden", border:"1px solid rgba(130,10,209,0.3)" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ background:"var(--bg3)", padding:"18px 20px 14px", borderBottom:"1px solid var(--border)", position:"relative" }}>
          <button onClick={onClose} style={{ position:"absolute", top:12, right:14, background:"var(--bg2)", border:"1px solid var(--border)", color:"var(--text2)", width:26, height:26, borderRadius:"50%", fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
          <div style={{ display:"inline-block", background:"rgba(130,10,209,0.2)", color:"#c084fc", fontSize:10, fontWeight:700, letterSpacing:0.8, padding:"3px 9px", borderRadius:20, border:"0.5px solid rgba(130,10,209,0.3)", marginBottom:10, textTransform:"uppercase" }}>Apoie o projeto</div>
        ...
        <div style={{ padding:"16px 20px" }}>
          ...
          <div style={{ background:"var(--bg3)", border:"1px solid var(--border)", borderRadius:10, padding:"13px", marginBottom:10 }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:10 }}>
              ...
              <span style={{ fontSize:11, color:"var(--text2)", fontWeight:700, letterSpacing:0.3 }}>PIX · Nubank</span>
            </div>
...
// Hook para controlar exibição do pop-up de doação
function useDonationPopup(createdAt?: string) {
  ...
  useEffect(() => {
    ...
    if (daysSince < 7) return; // Menos de 7 dias → não mostra

    const lastShown = localStorage.getItem("mg_donation_last");
    const lastTs = lastShown ? parseInt(lastShown) : 0;
    const daysSinceLast = (now - lastTs) / 86400000;
    if (daysSinceLast < 7) return; // Já mostrou há menos de 7 dias

    // Conta exibições do mês
    const currentMonth = new Date().toISOString().slice(0,7);
    const monthKey = `mg_donation_month_${currentMonth}`;
    const showsThisMonth = parseInt(localStorage.getItem(monthKey) || "0");
    if (showsThisMonth >= 4) return; // Máximo 4x por mês

    // Mostra após 2s
    const t = setTimeout(() => {
      setShow(true);
      localStorage.setItem("mg_donation_last", String(now));
      localStorage.setItem(monthKey, String(showsThisMonth + 1));
      // Avança o índice da mensagem
...
function RankingContent({ currentUserId }: { currentUserId: number }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize:17, fontWeight:800, marginBottom:4 }}>🏆 Ranking Global</h2>
      <div style={{ fontSize:12, color:"var(--text2)", marginBottom:14 }}>Top jogadores por XP total</div>

      {loading ? (
        <div style={{ textAlign:"center", color:"var(--text2)", padding:40 }}>Carregando...</div>
      ) : ranking.length === 0 ? (
          ...
          {ranking.map((u: any, i: number) => {
            ...
            return (
              <div key={u.id} style={{
                ...
                background: isMe ? "rgba(130,10,209,0.12)" : "var(--bg2)",
                border: `1px solid ${isMe ? "rgba(130,10,209,0.4)" : "var(--border)"}`,
                borderRadius:12
              }}>
                <div style={{ fontSize:18, minWidth:28, textAlign:"center", fontWeight:800, color: i < 3 ? "var(--yellow)" : "var(--text2)" }}>{medal}</div>
                <div style={{ fontSize:22 }}>{u.emoji || "💸"}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700, fontSize:14, color: isMe ? "#c084fc" : "var(--text)" }}>
                    {u.nickname || u.name?.split(" ")[0] || "Jogador"}
                    {isMe && <span style={{ fontSize:10, color:"#c084fc", marginLeft:6 }}>• você</span>}
                  </div>
                  <div style={{ fontSize:11, color:"var(--text2)", marginTop:2 }}>
                    <span style={{ color:levelInfo.color, fontWeight:700 }}>{levelInfo.label} NV.{levelInfo.levelNum}</span>
                    {u.streakDays > 0 && <span style={{ marginLeft:8 }}>🔥 {u.streakDays} dias</span>}
                  </div>
                </div>
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:15, fontWeight:800, color:"var(--primary)", fontVariantNumeric:"tabular-nums" }}>{(u.xp||0).toLocaleString("pt-BR")}</div>
...
tip=positivo/verde, alert=atenção/vermelho, info=informativo/roxo, gold=projeção/dourado.`;
  ...
  return (
    <div style={{ marginBottom:14 }}>
      {!insights && (
        <div style={{ width:"100%", background:"linear-gradient(135deg,rgba(130,10,209,0.1),rgba(0,214,143,0.06))", border:"1px solid rgba(130,10,209,0.25)", borderRadius:16, padding:"18px 16px", textAlign:"center" }}>
          <div style={{ fontSize:28, marginBottom:6 }}>🤖</div>
          <div style={{ fontSize:15, fontWeight:900, color:"var(--text)", marginBottom:4 }}>
            Análise IA — {monthLabel}
          </div>
          <div style={{ fontSize:12, color:"var(--text2)", marginBottom:12 }}>
            Gerada com base nos seus dados reais · atualiza todo mês
          </div>
          <button onClick={generate} disabled={loading}
            style={{ background:loading?"var(--bg3)":"linear-gradient(135deg,#820AD1,#6c63ff)", color:loading?"var(--text2)":"white", border:"none", borderRadius:12, padding:"12px 24px", fontSize:14, fontWeight:800, cursor:loading?"default":"pointer", width:"100%" }}>
            {loading ? "⏳ Analisando seus dados..." : "🤖 Gerar análise do mês"}
          </button>
        </div>
      )}
...
function SettingsModal({ user, salary, onSave, onClose, onReset, onOpenEmoji, userEmoji, userNick }: any) {
    ...
    <Modal title="⚙️ Configurações" onClose={onClose}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ borderTop:"1px solid var(--border)", paddingTop:14 }}>
          <div style={{ fontSize:12, color:"var(--text2)", marginBottom:8 }}>🎮 Identidade no ranking</div>
          <button onClick={onOpenEmoji} style={{ width:"100%", background:"rgba(130,10,209,0.08)", border:"1px solid rgba(130,10,209,0.25)", color:"#c084fc", padding:"10px 12px", borderRadius:10, fontSize:13, textAlign:"left", cursor:"pointer", display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:20 }}>{userEmoji||"💸"}</span>
            <span>{userNick || "Definir apelido e emoji"}</span>
          </button>
        </div>
        <div style={{ borderTop:"1px solid var(--border)", paddingTop:14 }}>
